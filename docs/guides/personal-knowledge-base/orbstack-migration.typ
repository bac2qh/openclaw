#set document(title: "Migrate from Lume VM to OrbStack Ubuntu")
...
Install OrbStack from #link("https://orbstack.dev")[orbstack.dev] if not already installed, then:
...
OrbStack Ubuntu supports systemd, so Xvfb can run as a proper service. Apply the settings from #link("./browser-cloudflare-stealth.md")[browser-cloudflare-stealth.md]:
